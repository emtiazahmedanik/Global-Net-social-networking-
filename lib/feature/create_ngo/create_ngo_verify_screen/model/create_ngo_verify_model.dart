...
/// Generic model to represent either a Community or an NGO.
/// When parsing, pass `isNgo: true` for NGOs so ngoType is read,
/// otherwise pass `isNgo: false` for Community (reads communityType).
class OrganizationModel {
  ...
  final bool isNgo; // true -> parsed from ngo endpoint
...
class ProfileModel {
  final String id;
  final String? parentId; // communityId or ngoId (keep generic)
  final String? name;
  ...
  factory ProfileModel.fromJson(Map<String, dynamic> json) {
    // There might be either 'communityId' or 'ngoId'
    final parentId = json['communityId'] ?? json['ngoId'] ?? json['groupId'];
    return ProfileModel(
      id: json['id'] as String,
  ...
  Map<String, dynamic> toJson() {
    return {
      "id": id,
      // parentId key not included as we don't know whether to write communityId or ngoId
      "name": name,
      "username": username,
      "title": title,
      "bio": bio,
